...
Node *RRotate(Node *y){   ////sað döndürme
	Node *x=y->left;
	Node *t=x->right;
...
Node *LRotate(Node *x){    ////sol döndürme
	Node *y=x->right;
	Node *t=y->left;
...
Node* EkleAVL(Node *node,int data){
	...
	////// 2.aþama yükseklik güncellemesi
	node->height=max(height(node->left),height(node->right))+1;
	
	////// 3.aþama denge kontrolü
